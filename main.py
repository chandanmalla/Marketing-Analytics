...
if selectbox=='1. Introduction':
    st.subheader('1.  How did we select an E-commerce platform for Marketing Strategy Assignment')
    st.write('Three criteria were set on which every E-commerce platform was analyzed')
    ...
    st.write('Based on the criteria and after having hours of discussion with the class, we settled on using data from consumers who shop from Flipkart')


    st.subheader('2. Evaluation of type of data to be analyzed')
    st.write('**A. We analyzed the invoice generated by Flipkart, and from those invoices, we found the following data points were helpful in the analysis:**')
    ...
    st.write('**B. We also captured psychometric and demographic data from consumers whom we gathered purchase data.**')
    ...
    st.subheader('3. How purchase data was gathered?')
    st.write('**A. We asked consumers to share their invoices either through email or download each invoice from Flipkart\'s website.**')
    st.write('Email:')
    ...
    st.write('1. We extracted ***Categories, Subcategories and Ratings*** data from every product from each website using ```BeautifulSoup.```')
    st.write('Following is the code we created to extract the required data:')
    st.code('import bs4\n'
...
'\t\tfor i in tqdm(range(data.shape[0])):\n'
   ...
   '\t\t\ttry:\n'
        ...
        '\t\t\trating = soup.find(\'div\', attrs={\'class\': \'_3LlK\'}).\n'
        '\t\t\tfor foo in soup.find_all(\'div\', attrs={\'class\': \'TBB\'}):\n'
            '\t\t\t\tbar = foo.find(\'a\', attrs={\'class\': \'_2M8F\'})\n'
            '\t\t\t\tif len(temp)>=2:\n'
                '\t\t\t\t\tbreak\n'
            '\t\t\t\telse:\n'
    ...
    st.write('2.**GST-Data** and **Delivery Data** was converted from absolute value to percentages ')
    st.write('3.**City_Tier** was extracted from Cities to which order was placed to analyze Tier wise behaviour')
    st.write('4 **Brand** was extracted from the Product name given  ')
    st.write('5   To get the **Sale** column, we compiled the information of every Flipkart Sale that happened since 2019 and compared it with the Order Date')
    st.write('6   To get **Covid** information we set the *1st wave* interval from **\'1-April-2020\' to \'1-September-2020\'** and *2nd Wave* from **\'1-Mar-2021\' to \'1-June-2021\'**'
             'and rest were set as **\'No\'**')
    st.write('7.**Month:** To determine the purchase pattern along the year, we segregated the orders according to purchase months(extraction of the month was from order date).')
    st.write('8.**Year:** To observe the purchase trends year on year, we extracted the year from the purchase date into the year and analyzed it.')
    st.write('9.**Time_month:** To observe the purchase behaviour during the start, mid and end of the month(extracted from order_date)')
    st.write('10.**Weekend/Weekday:** From the purchase date, we identified the day of the week when the purchase happened to observe trend change over the weekend.')
    st.write('11.**Log_Final Price:** To observe the purchase value distribution among consumers and better visualization, the purchase price was converted to a natural log.')
    st.write('12.**More than Average**: This feature was added to classify if a "buyer" was buying in more number of categories than "Average number of purchase categories per buyer( in our case, it was four .8)"')

    st.write('\n\n')
    st.write('***B. Survey Data***')
    st.write('1.**Age:** To analyze age-wise behaviour(extracted from Date of Birth)')
    ...
    st.write('3.**Collectiv**, **Life_Satsif**, **Indiv**, **LongTermOri**, **ShortTermOri**, **Materialism**, **Spiritualism**, **EnvBehav**: All these Behaviours were extracted '
         'by taking an average of the features in a document shared by Mam which was present in the survey')
    st.write('4.**Has child:** Based on the "number of children" a buyer has, he was classified as "Yes or No" in this feature.')
    st.write('5.**Lives_in_City Tier:** To perform better demographic analysis, we segregated these purchase order cities into "Tiers".'
    ...
    st.write('6.**Cluster:** The buyers were separated into three clusters using "K-Prototype Clustering".')
    st.write('7.**Participated_in_Sale**: The buyers were separated into three groups who \' Never Participated\', \' Participated once\',\' Participated more than once\'')
    st.write('8.**More than Average**: This feature was added to classify if a "buyer" was buying in more number of categories than "Average number of purchase categories per buyer( in our case, it was 4 .8)"')
