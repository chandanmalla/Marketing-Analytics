...
if selectbox=='1. Introduction':
    ...
    st.write('**A. We asked consumers to share their invoices either through the email or download each invoice from Flipkart\'s website.**')
    st.write('Email:')
...
'\t\tfor i in tqdm(range(data.shape[0])):\n'
   ...
   '\t\t\ttry:\n'
    ...
    st.write('5   To get **Sale** column we compiled the information of every Flipkart Sale happened since 2019 and compared it with the Order Date')
    ...
    st.write('7.**Month:** To find out the purchase pattern along the year, we segregated the orders according to purchase months(extraction of month was from order date).')
    ...
    st.write('9.**Time_month:** To observe the purchase behaviour during start,mid and end of month(extracted from order_date)')
    st.write('10.**Weekend/Weekday:** From the purchase date, we identified day of the week when purchase happened, to observe trend change over the weekend.')
    st.write('11.**Log_Final Price:** To observe the purchase value distribution among consumers and better visualization, the purchase price was converted to natural log.')
    st.write('12.**More than Average**: This feature was added to classify if a "buyer" was buying in more number of categories than "Average number of purchase categories per buyer( in our case it was 4 .8)"')
    ...
    st.write('4.**Collectiv**, **Life_Satsif**, **Indiv**, **LongTermOri**, **ShortTermOri**, **Materialism**, **Spiritualism**, **EnvBehav**: All these Behaviours were extracted '
             'by taking average of the features in document shared by Mam which were present in survey')
